...
export class SubscriptionController extends GenericController<typeof Subscription, ISubscription> {
    ...
    subscribe = catchAsync(async (req: Request, res: Response) => {
        ...
        if(!plan){
          throw new ApiError(StatusCodes.BAD_REQUEST, `plan is not provided in query, it should be ${Object.values(
                    SubscriptionType
                  ).join(', ')}`);
        }

        if(!userId){
          throw new ApiError(StatusCodes.BAD_REQUEST, `User must be logged in to subscribe`);
        }

        // check if plan is valid
        const validPLan = await subscriptionService.getBySubscriptionType(plan as string);

        if(!validPLan){
          throw new ApiError(StatusCodes.BAD_REQUEST, `Invalid plan provided in query, it should be ${Object.values(
                    SubscriptionType
                  ).join(', ')}`);
        }
        ...
        const session = await this.stripe.checkout.sessions.create({
          ...
          if(!session.url){
            throw new ApiError(StatusCodes.BAD_REQUEST, `Unable to create subscription session`);
          }
